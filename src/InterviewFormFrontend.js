...
import InterviewReportFrontend from "./InterviewReportFrontend";
...
const InterviewFormFrontend = () => {
  ...
  return (
    ...
    <div style={styles.container}>
      ...
      {formData.react_questions.questions.map((q, index) => (
        <div key={index}>
          <label>Question:
            <input type="text" name="question" value={q.question} onChange={(e) => handleChange(e, "react_questions", "questions", index)} />
          </label>
          <label>Response:
            <textarea name="response" value={q.response} onChange={(e) => handleChange(e, "react_questions", "questions", index)} />
          </label>
        </div>
      ))}
      ...
      <label>
        Evaluation:
        <select name="evaluation" value={formData.react_questions.evaluation} onChange={(e) => handleChange(e, "react_questions")}>
          <option value="">Select</option>
      ...
      <label>
        Score (Out of 10):
        <input type="number" max="10" min="0" name="score" value={formData.react_questions.score} onChange={(e) => handleChange(e, "react_questions")} />
      </label>


      <h3>JS Questions</h3>
      {formData.js_questions.questions.map((q, index) => (
        <div key={index}>
          <label>Question:
            <input type="text" name="question" value={q.question} onChange={(e) => handleChange(e, "js_questions", "questions", index)} />
          </label>
          <label>Response:
            <textarea name="response" value={q.response} onChange={(e) => handleChange(e, "js_questions", "questions", index)} />
          </label>
        </div>
      ))}
      <button onClick={addQuestion}>Add Question</button>
      <label>
        Evaluation:
        <select name="evaluation" value={formData.js_questions.evaluation} onChange={(e) => handleChange(e, "js_questions")}>
          <option value="">Select</option>
          <option value="Strong">Strong</option>
          <option value="Moderate">Moderate</option>
          <option value="Needs Improvement">Needs Improvement</option>
      ...
      <InterviewReportFrontend report={formData} />
      </div>
      </div>
      </div>
  );
...
export default InterviewFormFrontend;
